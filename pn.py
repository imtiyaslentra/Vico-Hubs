...
-- Periksa apakah bola memiliki kecepatan yang cukup rendah atau bergerak vertikal terlalu tinggi
...
-- Menghitung kecepatan pemain
local player_Velocity = local_player.Character.HumanoidRootPart.AssemblyLinearVelocity
local player_isMoving = player_Velocity.Magnitude > 0 -- Apakah pemain bergerak?

-- Batasi kecepatan bola untuk menghindari efek kurva
local ball_speed_Limited = math.min(ball_Speed / 1000, 0.1)

-- Mendapatkan posisi target dari entitas terdekat dengan kursor
local targetPosition = closest_Entity_To_mouse.HumanoidRootPart.Position
local target_Distance = local_player:DistanceFromCharacter(targetPosition)

-- Jarak antara pemain dengan entitas terdekat lainnya
...
local target_isMoving = target_Velocity.Magnitude > 0 -- Apakah target bergerak?

-- Dot product untuk menentukan seberapa sejajar arah bola dengan pergerakan target
local target_Dot = target_isMoving and math.max(target_Direction:Dot(target_Velocity.Unit), 0) or 0

-- Mengatur jarak spam berdasarkan ping dan kecepatan bola
aura_table.spam_Range = math.clamp(math.max(math.max(ping / 8.5, 15) + ball_Speed / 7.5, 8.4), 25, 1200)

-- Menghitung faktor kecepatan berdasarkan ping
local speedFactor = 3 - (ping / 100)
local speedFactorPingBased = 6.1 - (ping / 100)

-- Mengatur jarak parry berdasarkan ping dan kecepatan bola
aura_table.parry_Range = math.max(math.max(ping, 3.5) + ball_Speed / speedFactor, speedFactorPingBased)

-- Menentukan apakah spamming terjadi
aura_table.is_Spamming = aura_table.hit_Count > 1 or (target_Distance <= aura_table.spam_Range and ball_Distance < 18 and ball_Speed > 8) and ball_Velocity.Y > -50 and target_Dot > 0.15

-- Fungsi yang berjalan secara paralel untuk mengelola status ball warping
task.spawn(function()
    ...
    -- Periksa apakah ada target terakhir
    if aura_table.last_target then
        local last_target_Distance = (ball_Position - aura_table.last_target.HumanoidRootPart.Position).Magnitude
        if last_target_Distance > 35.5 or target_Distance <= 12 then
            aura_table.is_ball_Warping = false
        end
    end

    -- Mengatur status ball warping
...
Cur:AddToggle('Anti Curve V2',false,function(state)
